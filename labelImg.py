...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def open_last_labeled_image(self):
        curPath = os.path.abspath(os.getcwd())
        if not os.path.exists(curPath + "\\last_image_log\\log.txt"):
            with open(curPath + "\\last_image_log\\log.txt", 'x'): pass
        file = open(curPath + "\\last_image_log\\log.txt", "r")
        truth = True
        for img in file.readlines():
            sep = img.split("$and$")
            if len(sep) == 3 and self.dir_name == sep[0].replace("\\", "/"):
                print("asdasd")
                truth = False
                filename = sep[1]
                self.cur_img_idx = int(sep[2])
    ...
    def save_last_imageid(self, imgName, imgId):
        curPath = os.path.abspath(os.getcwd())
        file = open(curPath + "\\last_image_log\\log.txt", "w+")
        files = file.readlines()
        for (i, file) in enumerate(files):
            content = file.split("$and$")
            if len(content) != 3 or content[0] == self.dir_name:
                files.remove(file)
            
        file.write(self.dir_name + "$and$" + imgName + "$and$" + str(imgId))
        file.close()
    ...
    def _save_file(self, annotation_file_path):
        self.save_last_imageid(self.m_img_list[self.cur_img_idx], self.cur_img_idx)
